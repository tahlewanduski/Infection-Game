...
class State:
    ...
    def getMoves(self,player): #set Move
        ...
        for i in range(len(self.board[player])):
            ...
            for c in range(len(cloneMasque)):
                j,k=cloneMasque[c]
                if j >=0 and k >=0 and j <= 6 and k <= 6 and (j,k) not in self.board[nextcolor] and (j,k) not in self.board[self.player]:
    ...
    def play(self, Move): #State
        if self.player == "R":
           nextcolor="B"
            ...
            nextcolor="R"
        if Move.pos_i_x == None and Move.pos_a_x == None:
             New_State = State(self.board,nextcolor,self.turn+1,self.nRed,self.nBlue,self.hasPassed+1)
             return New_State
        else:
             ...
             for i in range(len(self.board[nextcolor])):
                ...
                if (x,y) in Masque:
                    ...
                    if x >=0 and y >=0 and x <= 6 and y <= 6 and (x,y) not in cooNotPlay:
    ...
    def __eq__(self,state):
        if len(self.board)== len(state.board) and self.player == state.player:
